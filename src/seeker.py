    ...
    def update(self, *args):
        ...
        for ufo in self.ufos:
            ufo.setAccelerationX(
                (self.ship.x - ufo.x) * self.enemy_acceleration * random()
            )

            ufo.setAccelerationY(
                (self.ship.y - ufo.y) * self.enemy_acceleration * random()
            )

            if random() > (1 / 100):
                ufo.setVelocityX(
                    ufo.body.velocity.x
                    + self.max_speed * self.enemy_speed * (random() - 0.5)
                )
                ufo.setVelocityY(
                    ufo.body.velocity.y
                    + self.max_speed * self.enemy_speed * (random() - 0.5)
